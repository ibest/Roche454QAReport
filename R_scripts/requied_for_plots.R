...
read.length.dataframe<-as.data.frame(read.length)

#histogram of read lengths 
#hist<-qplot(read.length, data=read.length.dataframe, geom="histogram")
#hist + geom_histogram(colour = "blue", fill = "white") + scale_x_continuous('read lenght')
...
#create a mean,sd of the means for each read 
...
#create an index to plot against in ggplot2
qual.length.index<-c(1:max.read.length)
...
base.qual.dataframe<-as.data.frame(cbind(mean.base.qual, sd.base.qual, qual.length.index))
colnames(base.qual.dataframe)<-c("mean.base.qual","SD","length.index")

the.plot<-ggplot(data=base.qual.dataframe, aes(qual.length.index, mean.base.qual,))
the.plot<-the.plot +geom_line()
the.plot<-the.plot +scale_x_continuous("b.p.") 
the.plot<-the.plot +scale_y_continuous("avg quality @ bp")
the.plot<-the.plot +geom_ribbon(aes(ymin=mean.base.qual-sd.base.qual, ymax=mean.base.qual+sd.base.qual), alpha=.1, fill="blue") 
the.plot<-the.plot + opts(title="B.P. quality & S.D.") 
